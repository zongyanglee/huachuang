...
def load_sheet(path: Path, sheet_name: str, start_date: pd.Timestamp) -> pd.DataFrame:
    raw = pd.read_parquet(path, columns=[BOND_CODE, TRADE_DATE, sheet_name])
    raw[TRADE_DATE] = pd.to_datetime(raw[TRADE_DATE])
    raw = raw.loc[raw[TRADE_DATE] >= start_date]
    return raw.pivot(index=BOND_CODE, columns=TRADE_DATE, values=sheet_name)
...
def prepare_core_data() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.Series, pd.Series]:
    ...
    pure_premium = concat_sheet("纯债溢价率", START_2015) / 100
    parity = concat_sheet("平价", START_2015)
    balance = concat_sheet("余额", START_2015)

    common_index = close.index.intersection(premium.index).intersection(pure_premium.index).intersection(parity.index).intersection(balance.index)
    common_cols = close.columns.intersection(premium.columns).intersection(pure_premium.columns).intersection(parity.columns).intersection(balance.columns)
    close = close.loc[common_index, common_cols]
    premium = premium.loc[common_index, common_cols]
    pure_premium = pure_premium.loc[common_index, common_cols]
    ...
    pure_premium = apply_listing_mask(pure_premium, listing, last_trade)
    parity = apply_listing_mask(parity, listing, last_trade)
    balance = apply_listing_mask(balance, listing, last_trade)
    return close, premium, pure_premium, parity, balance, listing, last_trade
...
def build_premium_comparison(premium: pd.DataFrame, pure_premium: pd.DataFrame) -> pd.DataFrame:
    compare = pd.DataFrame(
        {
            "交易日期": pd.to_datetime(premium.columns),
            "平均转股溢价率": (premium.mean(axis=0, skipna=True) * 100).to_numpy(),
            "平均纯债溢价率": (pure_premium.mean(axis=0, skipna=True) * 100).to_numpy(),
        }
    )
    return compare[compare["交易日期"] >= START_2018].reset_index(drop=True)
...
def save_count_premium_chart(market: pd.DataFrame, output_dir: Path, font_prop: fm.FontProperties) -> Path:
    path = output_dir / "转债数量与平均转股溢价率.png"
    x = market["交易日期"]
    fig, ax_count = plt.subplots(figsize=(14, 7), dpi=180)
    ax_premium = ax_count.twinx()
    ...
    ax_count.set_ylabel("转债数量", fontproperties=font_prop, fontsize=16, color="black")
    ax_premium.set_ylabel("平均转股溢价率", fontproperties=font_prop, fontsize=16, color="black")
    ax_premium.yaxis.set_major_formatter(PercentFormatter(xmax=1, decimals=0))
    style_axes(ax_count, font_prop)
    style_axes(ax_premium, font_prop)
    style_time_axis(ax_count, x)

    legend = ax_count.legend(
        [line_count, line_premium],
        ["转债数量", "平均转股溢价率"],
        loc="upper left",
        frameon=False,
        prop=font_prop,
...
def write_excel(
    ...
    group_avg_excel = group_avg.drop(columns=[col for col in group_avg.columns if "样本数" in str(col)], errors="ignore")
    index_excel = index_df[["交易日期", "日涨跌幅", "指数点位"]].copy()
    ...
    with pd.ExcelWriter(output_path, engine="openpyxl", datetime_format="yyyy-mm-dd") as writer:
        ...
        premium_compare.to_excel(writer, sheet_name="溢价率对比2018以来", index=False)
        group_avg_excel.to_excel(writer, sheet_name="平价分组均值2015以来", index=False)
        ...
        index_excel.to_excel(writer, sheet_name="余额加权指数", index=False)
        ...
        for ws in wb.worksheets:
            ...
            for col_idx, column_cells in enumerate(ws.columns, 1):
                ...
                if ws.title == "溢价率对比2018以来" and header in {"平均转股溢价率", "平均纯债溢价率"}:
                    for cell in column_cells[1:]:
...
def main() -> None:
    close, premium, pure_premium, parity, balance, listing, _ = prepare_core_data()
    market = build_market_stats(close, premium)
    premium_compare = build_premium_comparison(premium, pure_premium)
    ...
    chart_paths = [
        ...
        save_count_premium_chart(market, output_dir, font_prop),
    ...
    audit_rows = [
        ...
        ("溢价率对比口径", "2018年以来；同样清洗上市日前和最后交易日后的样本；平均转股溢价率与平均纯债溢价率均为算术均值；以百分数点输出，80代表80%"),
        ("数量与溢价率图", "左轴转债数量使用平均转股溢价率的有效样本数；右轴为平均转股溢价率"),
    ...
    write_excel(excel_path, market, premium_compare, group_avg, group_pct, group_pct_smooth, index_df, commentary_df, audit_df)
