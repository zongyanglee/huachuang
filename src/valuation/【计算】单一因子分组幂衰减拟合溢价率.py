"""使用纯幂衰减模型计算分组拟合溢价率时间序列。

脚本内置数据读取、样本清洗、窗口构建和四参数幂衰减拟合，不使用任何
截面残差修正因子。除原有分组结果外，同时计算单日全市场截面的“幂衰减
百元拟合溢价率”。默认复用本脚本的独立历史缓存，仅计算缺失或新增交易
日，并输出 Excel、历史曲线和最新截面散点拟合图。
"""
...
DEFAULT_OUTPUT_DIR = WORKSPACE / "runs" / "research" / f"幂衰减拟合溢价率_{RUN_DATE}"
RESULT_CACHE_DIR = WORKSPACE / "tmp/cache/power_decay_fit"
DEFAULT_RESULT_CACHE_PATH = RESULT_CACHE_DIR / "幂衰减拟合溢价率历史.parquet"
DEFAULT_WEEKLY_MATURITY_CACHE_PATH = RESULT_CACHE_DIR / "剩余期限周分组.parquet"
INDEX_BACKUP_PATH = RESULT_CACHE_DIR / "指数_写入幂衰减拟合前备份.parquet"
INDEX_MANIFEST_BACKUP_PATH = RESULT_CACHE_DIR / "sheet_manifest_写入幂衰减拟合前备份.parquet"
LEGACY_RESULT_PATH: Optional[Path] = None
...
WRITE_DAILY_FITS_TO_INDEX = False
...
INDEX_NAME = "指数名称"
INDEX_VALUE = "指数值"
POWER_DECAY_ANCHOR = 50.0
POWER_DECAY_LOWER = 50.0
POWER_DECAY_UPPER = 200.0
POWER_DECAY_MIN_SAMPLES = 36
ALL_MARKET_CATEGORY = "百元拟合"
POWER_DECAY_HUNDRED_GROUP = "幂衰减百元拟合溢价率"
ALL_MARKET_GROUPS = (POWER_DECAY_HUNDRED_GROUP,)
ALL_MARKET_INDEX_NAMES = {
    POWER_DECAY_HUNDRED_GROUP: POWER_DECAY_HUNDRED_GROUP,
}
REQUIRED_METRICS = [
    "平价",
    "转股溢价率",
...
def _format_power_decay_formula(
    amplitude: float,
    scale: float,
    power: float,
    floor: float,
    ...
    return (
        ...
        f"(1 + (平价 - {POWER_DECAY_ANCHOR:.2f}) / {scale:.2f})^(-{power:.2f})"
    )


def _empty_power_decay_fit_row() -> dict:
    return {
        "目标平价幂衰减溢价率": float("nan"),
        "样本数": 0,
        "R2": float("nan"),
        "RMSE": float("nan"),
        "拟合公式": "",
...
def _fit_power_decay_detail(
    df_subset: pd.DataFrame,
    target_x: float = 100.0,
    include_diagnostics: bool = False,
) -> dict:
    if df_subset is None or df_subset.empty:
        return _empty_power_decay_fit_row()
    required_cols = ["平价", "转股溢价率"]
    work = df_subset[required_cols].copy()
    work = work.replace("", np.nan).replace(0, np.nan)
    for column in required_cols:
        work[column] = pd.to_numeric(work[column], errors="coerce")
    work = work.dropna(subset=["平价", "转股溢价率"])
    if len(work) < POWER_DECAY_MIN_SAMPLES:
        return _empty_power_decay_fit_row()
    ...
    try:
        ...
        sse = float(np.sum(np.power(y - base_pred, 2)))
        rmse = float(np.sqrt(np.mean(np.power(y - base_pred, 2))))
        target_value = float(
            power_decay_with_floor(
                float(target_x),
                amplitude,
        ...
        row = {
            "目标平价幂衰减溢价率": target_value,
            "样本数": int(len(work)),
            "R2": float(1 - sse / sst) if sst > 0 else float("nan"),
            "RMSE": rmse,
            "拟合公式": _format_power_decay_formula(
                amplitude,
                scale,
                power,
                floor,
...
def _model_signature_payload(include_all_market: bool = True) -> dict:
    ...
    payload = {
        ...
        "model": "四参数幂衰减曲线",
        "minimum_samples": POWER_DECAY_MIN_SAMPLES,
        "parity_range": [POWER_DECAY_LOWER, POWER_DECAY_UPPER],
        "power_anchor": POWER_DECAY_ANCHOR,
    ...
    if include_all_market:
        payload["all_market_hundred_fits"] = {
            ...
            "target_parity": 100.0,
            "power_decay": {
                "formula": "floor + amplitude*(1+(x-anchor)/scale)^(-power)",
                "minimum_samples": POWER_DECAY_MIN_SAMPLES,
                "parity_range": [POWER_DECAY_LOWER, POWER_DECAY_UPPER],
            },
            "turnover_upper": 50,
...
def _daily_fit_index_name(category: str, group: str) -> str:
    ...
    return f"幂衰减拟合溢价率_{category}_{group}"
...
def _fit_all_market_hundred(base: pd.DataFrame) -> dict[str, dict]:
    ...
    power_decay_sample = base.loc[
        turnover_mask
        & base["平价"].gt(POWER_DECAY_LOWER)
        & base["平价"].lt(POWER_DECAY_UPPER)
    ].copy()
    power_decay_sample = _winsorize_premium_only(power_decay_sample)
    power_decay_detail = _fit_power_decay_detail(
        power_decay_sample,
        target_x=100.0,
    )

    return {
        POWER_DECAY_HUNDRED_GROUP: {
            "分类": ALL_MARKET_CATEGORY,
            "分组": POWER_DECAY_HUNDRED_GROUP,
            "目标平价": 100.0,
            "拟合溢价率": power_decay_detail["目标平价幂衰减溢价率"],
            "样本数": power_decay_detail["样本数"],
            "R2": power_decay_detail["R2"],
            "RMSE": power_decay_detail["RMSE"],
            "拟合公式": power_decay_detail["拟合公式"],
        },
    }
...
def _build_weekly_maturity_cross_section(
    ...
    common_mask = (
        ...
        & (parity > POWER_DECAY_LOWER)
        & (parity < POWER_DECAY_UPPER)
        & (turnover < 50)
        & (maturity >= 0)
...
def calculate_weekly_maturity_fit(
    ...
    for week in tqdm(
        ...
        if week in grouped.indices:
            ...
            detail = _empty_power_decay_fit_row()
        rows.append(
            {
                "周序号": week,
...
def plot_category_series(category: str, wide: pd.DataFrame, save_path: Path) -> None:
    ...
    if category == ALL_MARKET_CATEGORY:
        title = f"百元拟合溢价率：幂衰减（{start_year}年以来）"
        methodology = (
            "口径：单日全市场截面并读取平价100处的幂衰减拟合值；"
            "使用平价50—200且至少36条样本；"
            "换手率<50%，剔除溢价率两端各3%。"
        )
    else:
        title = f"{category}：幂衰减拟合溢价率（{start_year}年以来）"
        methodology = (
            "口径：四参数幂衰减曲线，不使用截面残差修正因子；"
...
def plot_weekly_maturity_fit(
    ...
    ax.set_title("剩余期限周分组：幂衰减拟合溢价率", loc="left", fontsize=15)
    ax.set_ylabel("拟合转股溢价率（%）")
    ...
    fig.text(
        ...
        "口径：四参数幂衰减曲线；平价50—200，换手率<50%，剔除溢价率两端各3%。",
        fontsize=9,
        color="#667085",
    )
    fig.tight_layout(rect=(0, 0.045, 1, 1))
...
def plot_latest_power_decay_cross_section(
    config: RunConfig,
    latest_date: pd.Timestamp,
) -> Path:
    """绘制最新单日截面的散点与纯幂衰减拟合曲线。"""
    frames, industry, all_dates = _prepare_data(config)
    ...
    if diagnostics is None:
        raise RuntimeError(
            f"{latest_date:%Y-%m-%d} 有效样本不足，无法绘制幂衰减拟合截面图"
        )
    ...
    curve_lower = max(0.0, POWER_DECAY_ANCHOR - scale + 0.01)
    ...
    r2 = detail["R2"]
    ax.set_title(
        f"{latest_date:%Y-%m-%d} 幂衰减拟合：R2={r2:.3f}",
        loc="left",
        fontsize=15,
    )
    ax.set_xlabel("平价")
    ...
    save_path = (
        config.output_dir
        / f"{latest_date:%m%d}【华创固收】幂衰减散点拟合.jpg"
    )
    fig.savefig(save_path, dpi=220, bbox_inches="tight", facecolor="white")
    plt.close(fig)
...
def save_outputs(
    ...
    excel_path = config.output_dir / f"幂衰减拟合溢价率_{RUN_DATE}.xlsx"
    ...
    summary = {
        ...
        "明细行数": int(len(result)),
        "模型": "分组幂衰减拟合溢价率 + 全市场幂衰减百元拟合溢价率",
        "模型方法": "四参数幂衰减曲线，不使用截面残差修正因子",
        "分组窗口": "分板块10日，其余分组5日；全市场百元序列1日",
...
def main() -> None:
    ...
    if config.make_plots:
        latest_plot_path = plot_latest_power_decay_cross_section(
            config,
