...
PLOT_GROUPS = ["70-90", "90-110", "110-130"]
...
PLOT_COLORS = {name: color for name, color in zip(PLOT_GROUPS, COLOR_SEQUENCE)}
...
def main() -> None:
    ...
    result_df = pd.DataFrame(results).drop_duplicates("date").sort_values("date")
    for name, _, _ in GROUPS:
        result_df[name] = pd.to_numeric(result_df[name], errors="coerce").replace({pd.NA: np.nan})
    date_tag = latest_date.strftime("%Y%m%d")
    output_dir = ROOT / "runs" / "weekly" / f"鹏华周报{date_tag}"
    output_dir.mkdir(parents=True, exist_ok=True)
    csv_path = output_dir / "平价分组平均转股溢价率.csv"
    png_path = output_dir / "平价分组平均转股溢价率.png"
