...
def main() -> None:
    ...
    for file in files:
        ...
        missing = {BALANCE, CLOSE, AMOUNT} - set(df["__sheet_name"].dropna().unique())
        if missing:
        ...
        codes = amount.index.union(balance.index).union(close.index)
        amount = amount.reindex(codes)
        balance = balance.reindex(codes)
        close = close.reindex(codes)
        ...
        close_parts.append(to_timeseries(close.where(traded), dates))

        valid_index = traded.stack()
        valid_index = valid_index[valid_index].index
        if len(valid_index) > 0:
            part = pd.DataFrame(
                {
                    BALANCE: balance.stack().reindex(valid_index),
                    CLOSE: close.stack().reindex(valid_index),
                }
            ).reset_index()
            part.columns = ["转债代码", "日期", BALANCE, CLOSE]
            part["日期"] = pd.to_datetime(part["日期"]).dt.strftime("%Y-%m-%d")
            part = part[["日期", "转债代码", BALANCE, CLOSE]]
            long_parts.append(part)

    balance_ts = pd.concat(balance_parts, axis=0).sort_index()
    close_ts = pd.concat(close_parts, axis=0).sort_index()

    all_codes = sorted(set(balance_ts.columns).union(close_ts.columns))
    balance_ts = balance_ts.reindex(columns=all_codes)
    close_ts = close_ts.reindex(columns=all_codes)
    ...
    close_ts.reset_index().to_csv(close_csv, index=False, encoding="utf-8-sig")
    long_ts = pd.concat(long_parts, axis=0, ignore_index=True) if long_parts else pd.DataFrame()
    long_ts.to_csv(long_csv, index=False, encoding="utf-8-sig")
    ...
    summary = {
        ...
        "cleaning_rule": "仅保留同一代码、同一日期成交额>0的余额和收盘价；其余置为空。",
        ...
        "close_non_null_cells": int(close_ts.notna().sum().sum()),
        "long_row_count": int(len(long_ts)),
