...
XLSX_PATH = OUTPUT_DIR / "余额与涨跌幅时间序列_宽表_成交额大于零清洗.xlsx"
...
def add_note_sheet(writer: pd.ExcelWriter, summary: dict) -> None:
    note = pd.DataFrame(
        [
            ...
            ("清洗规则", "仅保留同一代码、同一日期成交额>0的余额和涨跌幅；其余置为空。"),
            ("宽表结构", "行=日期，列=转债代码；分别输出“余额”和“涨跌幅”两张表。"),
            ("余额有效单元格数", summary["balance_non_null_cells"]),
            ("涨跌幅有效单元格数", summary["return_non_null_cells"]),
        ],
        columns=["项目", "说明"],
    )
...
def main() -> None:
    ...
    returns = pd.read_csv(RETURN_CSV)
    ...
    with pd.ExcelWriter(XLSX_PATH, engine="openpyxl", datetime_format="yyyy-mm-dd") as writer:
        ...
        returns.to_excel(writer, sheet_name="涨跌幅", index=False)
        add_note_sheet(writer, summary)

    style_workbook(XLSX_PATH)
    print(XLSX_PATH)
    print(f"balance_shape={balance.shape} return_shape={returns.shape}")
